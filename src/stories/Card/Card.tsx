...
import { BsFillStarFill, BsLink45Deg, BsSuitHeartFill } from 'react-icons/bs';
...
const Card = ({ movie, minWidth, maxWidth, ShowPopularity }: CardProps) => {
  const [ isShowOption, setShowOption ] = useState(false);

  function handleOnMouse(state: boolean) {
    setShowOption(state);
  }
  const { backdrop_path, vote_average,
          original_title, original_name,
          release_date, id, popularity } = movie;
